...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    tail = torch.tensor(1e10, device=dists.device, dtype=dists.dtype).expand(
        dists[..., :1].shape
    )
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            writer = imageio.get_writer(moviebase + "rgb.mp4", fps=30, codec="libx264")
            for frame in to8b(rgbs):
        ...
        # 👇 就在这里 加上可视化 log，和上面的 if 平级
        if i % args.i_img == 0:
            with torch.no_grad():
                img_i = i_test[0]  # 任选一个固定视角
                pose = poses[img_i, :3, :4]
